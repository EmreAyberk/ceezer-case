...
export class PortfolioController {
  ...
  @ApiOperation({
    ...
    description: 'This operation generates a portfolio and returns how the volume is separated.',
  })
  @ApiResponse({
    status: 200,
  ...
  async generatePortfolio(@Body() dto: GenerateEntryDto): Promise<ISerializeResponse> {
    if (isNaN(dto.desiredVolumeInTons) || dto.desiredVolumeInTons <= 0) {
      throw new BadRequestException('Desired tons should be a positive number.');
    }

    return await this.portfolioService.generatePortfolio(dto.desiredVolumeInTons);
  }
